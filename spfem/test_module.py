...
class TestArgyris(unittest.TestCase):
    """Solve biharmonic problem with Argyris."""
    def runTest(self,verbose=False):
        m=fmsh.MeshTri()
        m.refine(3)

        e=felem.ElementGlobalArgyris(optimize_du=True)
        a=fasm.AssemblerGlobal(m,e)

        A=a.iasm(lambda ddu,ddv: ddu[0][0]*ddv[0][0]+ddu[1][0]*ddv[1][0]+
                                 ddu[0][1]*ddv[0][1]+ddu[1][1]*ddv[1][1],intorder=6)
